...
//suzhiguang,store for calibration
    g_tfa98xx = tfa98xx;
	return ret;
}
		...
		char *buf)
{
    struct tfa98xx *tfa98xx;
	int ret;
	int cal_profile = -1, mtpex_state = -1, spkr_count = 0;
	int state ,err;
	...
	pr_err("tfa98xx_state_show\n");

	cal_profile = tfaContGetCalProfile(tfa98xx->tfa);

	if (cal_profile >= 0) {
		tfa98xx->profile = cal_profile;
	} else {
		cal_profile = 0;
		pr_err("tfa get cal profile error\n");
	}
	...
	if (state != TFA_STATE_OPERATING) {
		pr_err("tfa Calibration must be started in operating state!\n");
		goto err_exit_mutex;
	}

	err = tfa_supported_speakers(tfa98xx->tfa, &spkr_count);
	if (err) {
		pr_err("tfa Getting the number of supported speakers failed\n");
		goto err_exit_mutex;
	}

	err = tfaRunColdStartup(tfa98xx->tfa, cal_profile);
	if (err) {
		pr_err("tfa It is not possible to set device into cold-boot state!\n");
		goto err_exit_mutex;
	}

	/* Startup the device, ending in initCF state */
	err = tfaRunSpeakerBoost(tfa98xx->tfa, 0, cal_profile);
	if (err) {
		pr_err("tfa It is not possible to start device up!\n");
		goto err_exit_mutex;
	}

	/* Check if CF is not in bypass */
	if (!tfa_cf_enabled(tfa98xx->tfa)) {
		pr_err("tfa It is not possible to calibrate with CF in bypass! \n");
		goto err_exit_mutex;
	}
    ...
    if (tfa_dev_mtp_get(tfa98xx->tfa, TFA_MTP_EX) ) {
		if (mtpex_state == 1) {
			pr_err("tfa DSP already calibrated.\n Calibration skipped, previous results loaded.\n");
			//mtpex_state= tfa_dev_mtp_get(devs[dev_idx], TFA_MTP_EX);
		}
    ...
    {
	    /* Go to the Operating state */
	    //tfa_dev_set_state(devs[dev_idx], TFA_STATE_OPERATING);
		/* calibrate */
...
#if 0
   imp[0] = (float)tfa98xx->tfa->mohm[0] / scaling_factor;

	if (spkr_count == 1) {
		pr_err("tfa Calibration value is: %2.2f ohm\n", imp[0]);
	}
	else {
		imp[1] = (float)tfa98xx->tfa->mohm[1] / scaling_factor;
		pr_err("tfa Calibration value is: %2.2f %2.2f ohm\n", imp[0], imp[1]);
	}
#else
		pr_err("tfa Calibration value is: %d ohm\n", tfa98xx->tfa->mohm[0]);
#endif

	mutex_unlock(&tfa98xx->dsp_lock);
	ret = sprintf(buf,"%d:%d",2,tfa98xx->tfa->mohm[0]);
	return ret;

err_exit_mutex:
	mutex_unlock(&tfa98xx->dsp_lock);
	ret = sprintf(buf,"%d:%d",2,0);
	return ret;
			     ...
			     const struct i2c_device_id *id)
{
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		if (ret < 0) {
			pr_err("tfa failed to read Revision register: %d\n",
				ret);
			//suzhiguang
			if (gpio_is_valid(tfa98xx->reset_gpio)) {
