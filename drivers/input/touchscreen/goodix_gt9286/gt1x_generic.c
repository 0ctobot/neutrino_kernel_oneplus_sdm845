		...
		size_t size, loff_t * ppos)
{
	...
	printk("GTP[READ]simple_read_from_buffer ok %d!",__LINE__);
	ptr_p =ptr;
	ptr_p += sprintf(ptr_p, "==== GT1X default config====\n");
	printk("GTP[READ]simple_read_from_buffer ok 3!");
	for (i = 0; i < gt1x_cfg_length; i++) {
		ptr_p += sprintf(ptr_p, "0x%02X,", gt1x_config[i]);
	...
	ptr_p += sprintf(ptr_p, "==== GT1X read from chip====\n");
	...
	for (i = 0; i < gt1x_cfg_length; i++) {
		ptr_p += sprintf(ptr_p, "0x%02X,", temp_data[i]);
		if (i % 10 == 9 && i !=  GTP_CONFIG_ORG_LENGTH)
			ptr_p += sprintf(ptr_p, "\n");
	...
	ptr_p += sprintf(ptr_p, "\n");
	/* Touch PID & VID */
	ptr_p += sprintf(ptr_p, "==== GT1X Version Info ====\n");

	gt1x_i2c_read(GTP_REG_VERSION, temp_data, 12);
	ptr_p += sprintf(ptr_p, "ProductID: GT%c%c%c%c\n", temp_data[0],
			temp_data[1], temp_data[2], temp_data[3]);
	ptr_p += sprintf(ptr_p, "PatchID: %02X%02X\n", temp_data[4], temp_data[5]);
	ptr_p += sprintf(ptr_p, "MaskID: %02X%02X\n", temp_data[7], temp_data[8]);
	ptr_p += sprintf(ptr_p, "SensorID: %02X\n", temp_data[10] & 0x0F);

	irq_desc = irq_to_desc(gt1x_i2c_client->irq);
	if (irq_desc) {
		ptr_p += sprintf(ptr_p, "IRQ: %d, irq_desc->disable-depth:%d\n",
				gt1x_i2c_client->irq, irq_desc->depth);
	}
	printk("GTP[READ]simple_read_from_buffer  %d ,strlen(ptr) %d!",__LINE__ ,(int)(ptr_p-ptr));
	if ((ptr_p-ptr) >=4096) {
		GTP_ERROR("[READ]buffer is out failed!");
		ret = -ENOMEM;
		goto err_out_momery;
	}
	ret = simple_read_from_buffer(page, size, ppos, ptr, ptr_p-ptr);
	if (ret < 0)
	{
		GTP_ERROR("[READ]simple_read_from_buffer failed!");
                return ret;
        }
	printk("[READ]simple_read_from_buffer ok %d!",__LINE__);
err_out_momery:
	if(ptr != NULL)
		kfree(ptr);
	return ret;
...
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
	...
	if (ver_info != NULL) {
		...
		snprintf(gt1x_fw_id, 20, "%06XP%04XM%02XS",
			 patch_id, mask_id >> 8, sensor_id);
	}
 ...
 * Return: 0--success,non-0--fail.
 */
static s32 gt1x_wakeup_notify_sleep(void)
{
...
/* finger touch event*/
	if (CHK_BIT(cur_event, BIT_TOUCH)) {
		...
		gt1x_touch_up(0);
                ...
                if(gtp_background_double_check == 1){
                    gtp_background_double_check = 0;
                    gtp_background_double_flag = 1;
	            gt1x_touch_detect_switch(SWITCH_ON);
                }
#endif // CONFIG_GTP_TOUCH_DETECT
#endif
...
#ifdef CONFIG_GTP_TOUCH_DETECT
int touch_detect_flag = 0;

void gt1x_touch_detect_switch(s32 on)
{
	if (SWITCH_ON == on) {
		gt1x_send_cmd(0x00, 0x01);
		touch_detect_flag = 1;
	} else if (SWITCH_OFF == on) {
		gt1x_send_cmd(0x00, 0x00);
	}
}
#endif
...
int gt1x_notify_resume(void)
{
	...
	ret = gt1x_wakeup_notify_sleep();
	if (ret < 0) {
		GTP_ERROR("Resume  notify failed.");
	}

	gt1x_halt = 0;
	gt1x_irq_enable();

	GTP_DEBUG("Resume notify end.");
	return 0;
}
